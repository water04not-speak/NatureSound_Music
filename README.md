...
- 为什么要封装 `AvPlayerBridge`：隔离播放器状态机复杂度，页面只关注业务动作。
- 为什么使用 Repository 层：避免页面直接操作数据库，降低数据访问耦合。
- 如何处理外链播放失败：统一 HTTPS 规范化，本地文件失败后回退远程播放。
- 如何做状态共享：使用 `AppStorageV2` 管理导航、用户、主题和播放状态。
- 如何体现项目差异化：加入非遗音乐地域内容，使项目不仅是通用音乐播放器。
- 如何继续优化：可扩展云端接口、真实用户系统、评论接口、播放历史、缓存清理和单元测试覆盖。
